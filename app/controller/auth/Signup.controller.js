...
const { SignupInitiateRequest, SignupVerifyRequest } = require('../../dto/req/signup.request');
const { SignupInitiateResponse, SignupVerifyResponse } = require('../../dto/res/signup.response');
...
exports.initiateSignup = async (req, res) => {
    try {
        ...
        const result = await signupService.initiateSignup(name, nickName, email, password);
        
        return res.status(200).json(
            ApiResponse.success(result.message, SignupInitiateResponse.create(result.data.email))
        );
    } catch (error) {
        return res.status(400).json(
...
exports.verifyOTP = async (req, res) => {
    try {
        // Create and validate DTO
        const verifyRequest = new SignupVerifyRequest(
            req.body.email,
            req.body.otp
        );
        ...
        return res.status(200).json(
            ApiResponse.success(result.message, SignupVerifyResponse.create(result.data))
        );
    } catch (error) {
        return res.status(400).json(
            ApiResponse.error(error.message)
...
exports.resendOTP = async (req, res) => {
    try {
        ...
        if (!email || email.trim().length === 0) {
            return res.status(400).json(
                ApiResponse.error('Email không được để trống')
            );
        }
        ...
        return res.status(200).json(
            ApiResponse.success(result.message, SignupInitiateResponse.create(email.trim().toLowerCase()))
        );
    } catch (error) {
        return res.status(400).json(
            ApiResponse.error(error.message)
